function reservoir(nodes,inputs,outputs,rbias,sparse,rout) {
    ...
    for (var i=0;i<nodes;i++)
    {
        ...
        for (var j=0;j<nodes;j++)
            if(i==j) { selfv = 1.0; tw.push(selfv); }
            else tw.push((Math.random()-0.5));
        weight.push(tw);
    ...
    this.predictstage3 = function(ostate,lrange,rrange,scaledrange) {
        ...
        var lp = ostate[lpt]; rp = ostate[lpt+1];
        var scale = 0;
        if (lp<0) scale = 1; else if (rp<0) scale = 0; else scale = -0.72134752*Math.log(0.5*lp/rp);
    ...
    this.buildresult = function(gostate,lrange,rrange,percent) {
        if (percent>1) percent=1;
        var posi=percent*(rrange-lrange-2)+0.5+lrange;
        for (var i=lrange;i<rrange;i++)
            gostate[i] = Math.exp(-(posi-i)*(posi-i)*onedist*onedist);
        return gostate;
    };
    this.postadjust = function(state,ostate,gostate) {
        ...
        for (var j=0;j<outputs;j++) {
            for (var i=0;i<nodes;i++)
                this.ow[j][i] = this.ow[j][i]*0.93 + rout * state[i] * delta[j];
            this.obias[j] = this.obias[j]*0.93 + rout * delta[j];
        }
    };
}
